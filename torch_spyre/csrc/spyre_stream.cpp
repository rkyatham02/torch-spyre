...
void SpyreStream::copyAsync(const at::Tensor& src,
  ...
  // Handle scalar tensors - unsqueeze to 1D for DMA
  ...
  if (host2device || device2host) {
    ...
    // Device-to-device copy - fallback to D2H + H2D
    // TODO(tmhoangt): optimize with direct D2D once flex runtime supports it
